...
- **Cross-run learning (opt-in, minimal)** — When `AAOS_PERSISTENT_MEMORY=1`, the `run-aaos.sh` launcher bind-mounts the host's `./memory/` into `/var/lib/aaos/memory`, so the SQLite episodic store and the stable Bootstrap ID survive restarts. The Bootstrap manifest instructs the agent to `memory_query` before decomposing a goal and `memory_store` a compact run summary after completion. Deliberately minimal: no new crate, no pattern schema, no reflection service — just existing primitives wired up. The plan is to observe 10-20 runs, then design a structured `PatternStore` only if recurring patterns justify one.
  - **Run 5 validated the protocol end-to-end.** Bootstrap queried memory before decomposing (empty on first run, as expected), spawned 4 children, and stored a run summary at completion — all under its stable ID, readable by the next run. Three side effects needed manifest tuning (shipped separately): children were also calling `memory_store` with their ephemeral IDs (orphaned writes); children with `file_write: /data/workspace/X/*` couldn't `file_list` the same dir (capability model requires a matching `file_read` glob); and Bootstrap followed `planning-and-task-breakdown` too rigidly, ignoring the skill's own "When NOT to use" section and doubling runtime vs a simpler flow. All three are now addressed in `manifests/bootstrap.yaml` without any runtime code changes.
