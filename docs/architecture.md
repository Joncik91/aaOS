...
**Which agents get worker confinement, honestly.** The plan executor spawns subtask agents **inline** via `execute_agent_for_subtask` — it does not go through `AgentBackend::launch`, so those agents have no broker session. `ToolInvocation::invoke` distinguishes `WorkerInvokeError::NoSession` (agent has no worker) from `Transport` (real broker failure) and falls back to daemon-side execution on `NoSession`, recording `execution_surface: Daemon` on the audit event so the CLI shows `[daemon]` honestly. **Net v1 scope:** sub-project 3's worker-side execution activates only for agents spawned explicitly via the `spawn_agent` tool (which goes through `backend.launch` and gets a worker session). Plan-executor subtasks remain daemon-side today. Closing that gap is its own sub-project — either (a) spawn every plan-executor subtask through `backend.launch` (cost: ~100ms per subtask for namespace + Landlock setup), or (b) refactor `execute_agent_for_subtask` to route through the namespaced backend explicitly.

**Capability re-check inside the worker.** Individual tool implementations (`FileReadTool::invoke` etc.) do their own `ctx.capability_registry.permits(...)` check. The worker's `InvocationContext` is intentionally minimal (empty registry, empty tokens) per the design principle "daemon-side has already authorized, worker re-check is redundant" — but the *tools themselves* still re-check against their local context, so worker-side execution of tools that read capability tokens from context will fail their own internal check. The one path that already works end-to-end is *Landlock-denial* outside the tool's own capability-check logic: the Landlock kernel boundary denies filesystem access that daemon-side capability would have permitted. A follow-up sub-project needs to either factor a `Tool::invoke_no_cap_check` variant or pass capability tokens across the broker so the worker can rebuild a minimal registry for the invoking agent. Tracked in roadmap for Phase F-b follow-ups.
