...
- **Plan-executor subtasks (LLM roles: analyzer, writer, generalist)** — spawn inline via `execute_agent_for_subtask`. **Opt-in** confinement via `AAOS_CONFINE_SUBTASKS=1`: when set, `run_subtask_inline` also launches a worker session. Default off because the worker's Landlock scope exposes only `/scratch` + shared libs — plans that read/write under `/var/lib/aaos/workspace/` (as every multi-step role does today) will get ENOENT inside the worker. Closing that gap needs workspace bind-mount design (see below).
- **Scaffold roles (fetcher, anything with `scaffold: {kind: ...}`)** — always run daemon-side. They ARE the workspace plumbing (fetch → write under workspace → hand-off). Confining them requires the same workspace-mount design as plan-executor subtasks.
- **Inline `NoSession` fallback** — `ToolInvocation::invoke` distinguishes `WorkerInvokeError::NoSession` (agent has no worker — legitimate) from `Transport` (real broker failure). On `NoSession`, fall back to daemon-side execution and record `execution_surface: Daemon` on the audit event so the CLI `[daemon]` tag is honest.
...
**Workspace-access design, open.** Today's `PolicyDescription { scratch: /scratch, shared_libs, broker_socket }` gives the worker a mount namespace where only the session tmpfs and shared libraries exist. Paths like `/var/lib/aaos/workspace/<run-id>/` are not visible. For multi-subtask workflows (the canonical goal's fetcher → analyzer → writer) to run confined, the backend needs to bind-mount a per-run workspace subpath into the worker at a known location. `AgentLaunchSpec.workspace_path` exists for this purpose but is not yet consumed by `launch_impl`. Its own sub-project.
