...
- **Runtime-issued, not cryptographically unforgeable** — Tokens are UUID-identified Rust structs (`CapabilityToken`), created exclusively by `AgentRegistry::issue_capabilities()`. Agents have no code path to construct tokens themselves. However, the tokens are not signed — anyone with Rust-level code execution inside `agentd` can fabricate a valid-looking token. Proper unforgeability via handle-based tokens (table-indexed opaque references) or HMAC signing is a deferred hardening item, needed before any design that transports tokens across process boundaries.
...
- **Scope of enforcement** — Bundled tools (in `aaos-tools`) check tokens at the call boundary. Third-party tool plugins are responsible for their own capability checks — the runtime hands them the token and the invocation context, but doesn't universally interpose.
