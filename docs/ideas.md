...
## Runtime-side confinement of tool execution for NamespacedBackend — **PROMOTED** to Phase F-b (2026-04-18)

- **Why promoted:** the rubber-duck Standard spec names "Abstracted Filesystem where agents cannot touch host OS files" as a core Agent Kernel primitive. `AAOS_DEFAULT_BACKEND=namespaced` today applies the sandbox to the worker subprocess but not to the LLM loop + tool invocations — a reader of the flag's name reasonably expects full confinement. That gap is structural to the Standard spec, not a "wait for a customer to ask" item.
- **Target shape (from the Phase F-b entry):** the worker's agent loop drives tool calls; requests route over the already-shipped post-handshake broker stream (commit `7f7894d`); `agentd` resolves capabilities against the host-owned registry and either executes host-side or delegates to a worker-local runtime for pure-compute tools. Transport is exercised; wiring real tool invocations through is the remaining work.

## Dynamic model routing — cost- and latency-aware switching — **PROMOTED** to Phase F-b (2026-04-18)

- **Why promoted:** the rubber-duck Standard spec names "Dynamic Resource Allocation" as an Agent-Kernel primitive (cheap model for simple, strong for reasoning, driven by live signals). Static-model-per-role is the minimum viable version; the Standard tier requires per-subtask selection.
- **Target shape (from the Phase F-b entry):** a `ModelPolicy` DSL on roles (`mechanical → deepseek-chat`, `design → deepseek-reasoner`, plus escalation rules), a classifier deciding which bucket a subtask falls into, and a cost/latency feedback loop from `BudgetTracker` informing future routing. Builds on the existing `BudgetTracker` and `ScheduledLlmClient`; no new crates.
...
- **Idea:** instead of shipping a Debian-derivative image (Phase F-c: upstream Debian installer + aaOS preinstalled + opinionated defaults), ship a fully custom installer — branded "aaOS," non-Debian installation flow, potentially immutable A/B-partition rootfs à la Bottlerocket / Fedora CoreOS / Talos.
