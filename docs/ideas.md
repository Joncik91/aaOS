...
## Self-hosted build loop (aaOS applies plans to aaOS) — **surface shipped 2026-04-17; tool-gap follow-ups identified**
...
- **Status:** tool + role shipped in commit `45ce06b`; self-build runs 1–6 on a DO droplet exposed three tool-layer gaps (documented in `docs/reflection/2026-04-17-self-build-tool-gap.md`). Runs 1–4 validated the happy path on a plan that was already implemented; runs 5–6 attempted a real fix and failed at the edit step because the tool surface can't express surgical edits.
- **Follow-ups before the next self-build run is worthwhile:**
  1. **`file_read` `offset` + `limit` parameters.** Today the tool returns the whole file (up to 1MB). The LLM has no navigation affordance — dumping a 100KB file gives it a wall of text it can't seek into. Mirror Claude Code's `Read { offset, limit }` signature.
  2. **`file_edit` tool.** Primitive shape: `{ path, old_string, new_string }` with a "must be unique" rule on `old_string`. This is the Edit/Write idiom every mainstream coding agent uses (Claude Code, Cursor, Aider). aaOS's `file_write` rewrites the entire file from a `content` param, which for any non-trivial edit on a large file blows the role's output-token budget.
  3. Don't solve either of the above by raising `max_output_tokens`. That's the wrong lever: giving the model 25k output tokens so it can rewrite a whole file is strictly worse than giving it a 2-argument surgical edit primitive.
- **Signal to reconsider scope:** once the three follow-ups above ship and a self-build run produces a clean diff that the author commits, expand the tool set with `git_commit` (narrow) and `rustfmt`. If the loop keeps diverging after that, the failure mode tells us what the next primitive is.
