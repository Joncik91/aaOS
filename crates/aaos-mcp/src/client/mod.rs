...
impl McpClient {
    ...
    pub async fn connect_and_register(config: &ClientConfig, registry: &ToolRegistry) -> Self {
        ...
        for server_cfg in &config.servers {
            ...
            let session = match McpSession::connect(server_cfg.name.clone(), transport).await {
                Ok(s) => s,
                Err(e) => {
                    tracing::warn!(
                        server = %server_cfg.name,
