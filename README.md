...
**What exists today:** A working agent runtime on Linux. 8 Rust crates, ~25,400 lines, 447 tests. Ships as a Debian `.deb` with an operator CLI (`agentd submit|list|status|stop|logs|roles`). Orchestration is two-phase: a cheap-LLM **Planner** emits a structured Plan; a deterministic Rust **PlanExecutor** walks the resulting DAG, running independent subtasks in parallel. The legacy Bootstrap-Agent-in-Docker path (autonomous single-LLM orchestrator) is still supported as a fallback when no role catalog is installed. Linux-namespaced agent backend scaffolding (Landlock + seccomp + peer-creds broker socket) is in place; kernel launch mechanics are pending manual verification on a 5.13+ host. The capability model, audit trail, and substrate-agnostic ABI date to Phase A and have survived ten self-reflection runs of the system reading its own code. See [`docs/reflection/`](docs/reflection/README.md) for what those runs surfaced and fixed.
...
- **aaos-tools** — Tool registry, invocation, 12 built-in tools (echo, web_fetch, file_read/list/read_many/write, spawn_agent/spawn_agents, memory_store/query/delete, skill_read)
...
- **aaos-backend-linux** — Opt-in Linux-namespaced agent backend (`namespaced-agents` feature). User/mount/IPC namespaces, Landlock + seccomp confinement, peer-creds-authenticated broker socket, worker subprocess. Kernel launch mechanics pending manual verification on a Linux 5.13+ host.
