...
**aaOS reads its own source through the same file/grep/edit tools agents use** — the self-reflection loop points an agent at the aaOS source tree on a fresh droplet, asks it to find bugs, the operator triages and ships fixes, the next run finds different bugs.  Across 9 numbered self-reflection rounds (March → April 2026) plus a libfuzzer pass and a 32-thread concurrency stress probe, the loop has surfaced 35 production defects (Bugs 1 through 35), all closed in `v0.0.x`–`v0.2.x`.  See [`docs/reflection/`](docs/reflection/README.md) for what each run surfaced.  ("Reads source code" means *its own* source under a capability grant — it does not read the Linux kernel or arbitrary system code unless explicitly granted.)

Droplet QA runs on a Debian 13 DigitalOcean instance when a release ships substantive changes that source-only tests can't validate (kernel-feature integration, packaging, namespaced backend confinement under real Landlock/seccomp, push-revocation under live broker IPC).  Smaller patch releases ship without a fresh droplet pass when CI + local testing cover the change surface.  See `docs/reflection/` entries tagged `*-droplet-qa.md` and `*-e2e-qa.md` for the explicit ones.
...
sudo apt install ./aaos_0.2.8-1_amd64.deb
...
- **Pluggable agent backend** — `InProcessBackend` for trusted hosts; `NamespacedBackend` with Linux user/mount/IPC namespaces + Landlock + seccomp. Default install runs under `InProcessBackend`; the `.deb`'s `postinst` probes for Landlock in `/sys/kernel/security/lsm` + unprivileged user namespaces and writes `/etc/default/aaos.example` with `AAOS_DEFAULT_BACKEND=namespaced` when both pass (Debian 13 stock kernel 6.12+ has both). Operators opt in by copying the line into the live `/etc/default/aaos` or setting the env var directly. Under namespaced, every plan-executor subtask's filesystem + compute tools execute inside the worker with capability tokens forwarded across the broker stream. Verified end-to-end on droplet under namespaced backend across the v0.0.x–v0.2.x line. Network + subprocess tools stay daemon-side by design (see [architecture](docs/architecture.md#worker-side-tool-confinement)).
