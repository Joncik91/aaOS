...
- **Capability-based security** — Runtime-issued tokens, zero-capability default for agents, two-level enforcement inside bundled tools (tool access + resource path). Parent agents can only delegate capabilities they hold — "you can only give what you have." Path canonicalization (filesystem-resolved, including symlinks) prevents traversal and symlink-bypass attacks. Child tokens inherit parent constraints. Tokens are revocable at runtime. **Scope of enforcement:** bundled tools check tokens at their call boundary. Third-party tool plugins must implement their own capability checks — the runtime provides the token, not a universal interposition layer. Tokens are Rust structs (not cryptographically signed); forgery would require code execution inside `agentd`, which is the same trust boundary as token issuance. Proper unforgeability via handle-based tokens or HMAC signing is a deferred hardening item.
...
2. **Capability-Based Security** — Agents start with zero capabilities. Runtime-issued tokens replace permissions; bundled tools check tokens at their call boundary. Agents cannot construct tokens themselves (only the runtime issues them), though tokens are not cryptographically unforgeable — a proper handle-based or HMAC-signed token system is a deferred hardening item.
