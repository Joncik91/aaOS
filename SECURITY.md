...
2. **Worker-side forgery on `NamespacedBackend` — closed, transport exercised.** The namespaced backend runs agent workers in Linux user/mount/IPC namespaces with Landlock + seccomp applied before the agent loop begins. Workers receive no handles in the launch protocol and the handle-field privacy prevents local fabrication. The broker socket is peer-creds-authenticated (`SO_PEERCRED` exact match on pid+uid+gid). The persistent post-handshake broker↔worker stream (`7f7894d`) carries correlated request/response traffic under the sandbox — verified end-to-end with `Ping` and `Poke` round-trips on Debian 13 / kernel 6.12.43 and CI's Ubuntu 24.04. Forgery at this layer is structurally impossible. Runtime-side confinement of tool execution (routing `file_read` / `file_write` / etc. through the same stream instead of host-executing) is a separate capability expansion tracked in [`docs/ideas.md`](docs/ideas.md#runtime-side-confinement-of-tool-execution-for-namespacedbackend).
